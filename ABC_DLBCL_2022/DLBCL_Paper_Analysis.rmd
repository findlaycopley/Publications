...
GenerateLinearPredictor <-
    function(betas ,
        ...
        if (scale) {
            ...
            FinObject$LinearPredictor <-
                scale(FinObject$LinearPredictor)
        }
...
pData(Data$PairsData) %>%
    pivot_wider(id_cols = PatID, names_from = Timepoint, values_from = COO) %>%
    subset(Diagnosis != Relapse) %>%
    arrange(Diagnosis)
```
...
CohortData <- list(
    ...
    Sex = subset(pData(Data$PairsData), Timepoint == "Diagnosis", select = Sex) %>%
        table(),
    Coo = subset(pData(Data$PairsData), select = c(COO, PatID, Timepoint)) %>%
        subset(COO != "NA") %>%
        pivot_wider(id_cols = PatID, names_from = Timepoint, values_from = COO) %>%
        column_to_rownames("PatID") %>%
        table(),
    Site = subset(pData(Data$PairsData), select = c(Nodal, PatID, Timepoint)) %>%
        mutate(Nodal = ifelse(Nodal, "Nodal", "Extranodal")) %>%
            mutate(Nodal = factor(Nodal, levels = c("Nodal", "Extranodal"))) %>% 
        pivot_wider(id_cols = PatID, names_from = Timepoint, values_from = Nodal) %>%
        column_to_rownames("PatID") %>%        
        table(),
    TreatmentResponse = subset(pData(Data$PairsData), Timepoint == "Diagnosis", select = Response) %>%
        table(),
    Treatment = table(subset(pData(Data$PairsData), Timepoint == "Diagnosis", select = TREATMENT))
)
CohortData
subset(pData(Data$PairsData), Timepoint == "Diagnosis" & Relapse_Time > 10)
```
...
```{r}
## Bind the data to the name of that row
Cohort <- rbind(cbind(pData(Data$PairsData)["Timepoint"],"Timepoint") %>% 
                ...
                cbind(pData(Data$PairsData)["COO"],"COO") %>% 
                    setNames(c("val", "yaxis")) %>% 
                    rownames_to_column(var="sample"),
                ## If less that 2 years  <2y if greater than 2 years  >2y
                ...
                cbind(pData(Data$PairsData)["Response"],"Response") %>% 
                    setNames(c("val", "yaxis")) %>% 
                    rownames_to_column(var="sample"),
                ## Break age at diagnosis up into 0-50, 50-60, 60-70, 70+
                cbind(pData(Data$PairsData) %>% 
                    ...
                    setNames(c("val", "yaxis")) %>%
                    rownames_to_column(var="sample"),
                cbind(ifelse(pData(Data$PairsData)["Nodal"] == TRUE, "Nodal", "Extranodal") %>% as.data.frame(),
                      "Nodal/extranodal") %>% 
                    setNames(c("val", "yaxis")) %>%
                    rownames_to_column(var="sample"),
    ...
    ggplot(data=.,aes(x=sample, y=factor(yaxis,
                                         levels=rev(c("Timepoint","COO","Nodal/extranodal","Relapse Time","Gender","Age","Response"))),
...
cooKey <- get_legend(
    ggplot() +
        ...
        scale_fill_manual(values = Cohort_Colours[c(3:5,8)], name = "COO") + CohortLegTheme
)
nodeKey <- get_legend(
    ggplot() +
        geom_raster(
        ...
        scale_fill_manual(values = Cohort_Colours[6:7], name = "Nodal/Extranodal")  + CohortLegTheme
)
t2rKey <- get_legend(
    ggplot() +
        geom_raster(
...
## COO stability
COOStability <-
    spread(pData(Data$PairsData)[, c("Timepoint", "COO", "PatID")], Timepoint, COO) %>%
    subset(Diagnosis != "NA" & Relapse != "NA")
COOStability$Diagnosis <-
    ifelse(
        COOStability$Diagnosis == "Unclassified",
        "UNC",
        COOStability$Diagnosis %>% as.character()
    )
COOStability$Relapse <-
    ifelse(
        COOStability$Relapse == "Unclassified",
        "UNC",
        COOStability$Relapse %>% as.character()
    )
...
COOStability$Diagnosis <-
    factor(COOStability$Diagnosis, levels = c("ABC", "UNC", "GCB"))
COO <- ggplot(subset(COOStability, value != 0),
    ...
    scale_x_discrete(limits = c("Diagnosis", "Relapse"),
                     expand = c(.05, .05)) +
    theme(
...
PCA_COO <- biplot(pca,
                  ...
                  pointSize = 3, 
                  lab= NA ) +
        scale_color_manual(name="",values=Colours[c(3:5,7)]) +
        theme_classic() +
        theme(legend.position = "bottom",
              legend.direction = "horizontal")
...
Create a column to indicate ABC-ABC and GCB-GCB pairs

```{r}
## Get Pat ID for samples that are ABC at both diagnosis and relapse
ABC_Pairs <- pData(Data$PairsData) %>%
    pivot_wider(id_cols = PatID,
                names_from = Timepoint,
                values_from = COO) %>%
    subset(Diagnosis == "ABC" & Relapse == "ABC") %>%
    .$PatID
## Get Pat ID for samples that are GCB at both diagnosis and relapse
GCB_Pairs <- pData(Data$PairsData) %>%
    pivot_wider(id_cols = PatID,
                names_from = Timepoint,
                values_from = COO) %>%
    subset(Diagnosis == "GCB" & Relapse == "GCB") %>%
    .$PatID
## Get Pat ID for samples that are NA at either diagnosis and relapse
NA_Pairs <- pData(Data$PairsData) %>%
    pivot_wider(id_cols = PatID,
                names_from = Timepoint,
                values_from = COO) %>%
    subset(Diagnosis == "NA" | Relapse == "NA") %>%
    .$PatID
Data$PairsData$PairCOO <-
    ifelse(
        Data$PairsData$PatID %in% ABC_Pairs,
        "ABC",
        ifelse(
            Data$PairsData$PatID %in% GCB_Pairs,
            "GCB",
            ifelse(Data$PairsData$PatID %in% NA_Pairs, "NA", "UNC")
        )
    )
```

```{r}
ABC <- Data$PairsData[,Data$PairsData$PairCOO == "ABC"]
## Build the design
ABC_design <- model.matrix(~ABC$PatID+ABC$Timepoint)
## DGE
...
## GCB analysis
GCB <- Data$PairsData[,Data$PairsData$PairCOO == "GCB"]
## Build the design
GCB_design <- model.matrix(~GCB$PatID+GCB$Timepoint)
## DGE
...
ABC_FoldChange %>% 
    select(t) %>% 
    arrange(-t) %>% 
    setNames(c("t_statistic")) %>%
    rownames_to_column("#Probe Set ID") %>%
...
## Print out the significant (FDR.q.val <= 0.1) gene sets from the GSEA analysis, ordered by NES
ABC_GSEA %>% subset(FDR.q.val <= 0.1) %>% arrange(- NES) %>% droplevels.data.frame() %>%  
        write.xlsx("GSEA/SigPathways.xlsx", sheetName="ABC")
## subset(GCB_GSEA, FDR.q.val <= 0.1)
...
GSEA_Down_Venn
## E
(ABC_GSEA_Heatmap |  GCB_GSEA_Heatmap ) + plot_layout(guides="collect")
## F
PAMR
```
...
## Compare the HMRN risk groups to the LACY class.
ColoursGenomic <- c(
        ## Same Groups
        "MCD" = "firebrick",
        "MYD88" = "firebrick",
...
y <- summary(x)
## % that have a lower HR than our signature
