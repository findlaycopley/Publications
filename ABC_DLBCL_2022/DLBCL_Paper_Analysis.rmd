...
pData(Data$PairsData) %>%
    pivot_wider(PatID, names_from = Timepoint, values_from = Nodal) %>%
    column_to_rownames("PatID") %>%
    table()

table(pData(Data$PairsData) %>%
    subset(Timepoint == "Diagnosis") %>%
    .$TREATMENT)
## Output for GEO
dir.create("DataStore")
Data$PairsData %>% exprs() %>% write.table("DataStore/all_batchCorrectedCounts.txt", sep = "\t", quote = FALSE, col.names = NA)
raw <- readRDS("RDS/rrDLBCL.raw.eSet.rds")
raw %>% exprs() %>% write.table("DataStore/all_rawCounts.txt", sep = "\t", quote = FALSE, col.names = NA)
lapply(colnames(raw), function(col) {
    raw[,col] %>% exprs() %>% write.table(paste("DataStore/", col, "_rawCounts.txt", sep = ""), sep = "\t", quote = FALSE, col.names = NA)
})
...
subset(GSEA, FDR.q.val.ABC <= 0.1 & FDR.q.val.GCB <= 0.1 & sign(NES.ABC) == sign(NES.GCB))
...
Genes  <- c(ABC_FoldChange %>% subset(P.Value <= 0.05) %>% rownames(), 
            GCB_FoldChange %>% subset(P.Value <= 0.05) %>% rownames()) %>% unique()
length(Genes)
cbind(Gene = Genes,
      ABC = ABC_FoldChange %>% .[Genes,"logFC"],
      GCB = GCB_FoldChange %>% .[Genes,"logFC"]) %>%
    as.data.frame() %>%
    ggplot(aes(x = as.numeric(ABC), y = as.numeric(GCB))) + 
    geom_point() + 
